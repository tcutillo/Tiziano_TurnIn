...
def main():
    ...
    header = ['contractID', 'contract Details', 'Position', 'Quantity', 'Price', 'Cash Allocation', 'SEA Rule']
    rows = []
    ...
    for conID in contractID:
        ...
        row = [conID, contractDetails[0].descAppend, "LONG", 10, data.close, (data.close*10)*10, getSeaRule((data.close*10)*10, months_between_dates('05/15/23', contractDetails[0].descAppend.split()[-1]))]
        print(row)
        rows.append(row)
